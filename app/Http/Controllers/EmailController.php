		...
		try {
	     ...
	     //        		'evawandamiranti15@gmail.com',
	            ...
	            // dd(asset('files/'.$_file_name));       	
		            	$target_email = $value;
		            	$target_name = 'Test yaaa';
	        			Mail::later(5, 'email.template', array('body_message' => $body_message, 'subject' => $subject, 'target_name' => $target_name ), function($message) use ($subject, $name, $email, $target_email, $target_name, $_file_name) {
	        				$message->from($email, $name);
	        				$message->to($target_email, $target_name)->subject($subject);
